...
class PredefinedFolderView implements VirtualDOM {
    ...
    constructor(
        ...
        const faClass = node.icon
        const title = node.name
        const defaultClasses = `d-flex align-items-center fv-pointer my-3 flex-grow-1 rounded`
        this.children = [
            {
                class: attr$(
                    state.currentFolder$,
                    ({ folder }) =>
                        folder.kind == node.kind &&
                        folder.groupId == node.groupId
                            ? 'fv-text-focus fv-hover-text-primary'
                            : 'fv-text-primary',
                    {
                        wrapper: (d) =>
                            `${d} ${defaultClasses} fv-hover-bg-secondary`,
                        untilFirst: `${defaultClasses} fv-text-primary fv-hover-bg-secondary`,
                    },
                ),
                children: [
                    {
                        class: attr$(extended$, (extended) =>
                            extended ? 'px-2 flex-grow-1 ' : 'd-none',
                        ),
                        innerText: title,
                    },
                    {
    ...
    constructor({
        ...
        this.children = children$(
            ...
            ({ folder }: { folder: DriveNode }) => {
                const tree = state.groupsTree[folder.groupId]
                const nodes = [
                    tree.getHomeNode(),
...
function formatGroups(groups: AssetsGateway.GroupResponse[]) {
    const isDirectParent = (child, maybeParent) => {
        return child.elements
            .slice(0, -1)
            .reduce(
                (acc, element, i) => acc && element == maybeParent.elements[i],
                true,
            )
    }
    const formatted = groups
        .map((group) => {
            const elements = group.path.split('/')
            return {
                name: elements.slice(-1)[0],
                level: elements.length - 1,
                elements,
                id: group.id,
            }
        })
    ...
    formatted.forEach((group) => {
        const index = sorted.findIndex((maybeParent) =>
            isDirectParent(group, maybeParent),
        )
        if (index) {
        ...
        this.children = [
            {
                ...
                children: [
                    this.headerView(),
                    child$(this.groupsExpanded$, (expanded) =>
                        expanded ? this.groupsView() : {},
                    ),
                ],
            },
        ]
            ...
            children: [
                {
                    ...
                    onclick: () =>
                        this.groupsExpanded$.next(
                            !this.groupsExpanded$.getValue(),
                        ),
                    children: [
                        {
                            class: attr$(
...
export class SideBarView implements VirtualDOM {
    ...
    constructor(
        ...
        this.style = attr$(extended$, (extended) =>
            extended
                ? {
                      width: '250px',
                  }
