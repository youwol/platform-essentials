...
export function createTreeGroup(groupName: string, respUserDrives, respDefaultDrive) {
    ...
    let systemFolderNode = new FolderNode<'system'>({
        ...
        parentFolderId: respDefaultDrive.driveId,
        children: RequestsExecutor.getFolderChildren(respDefaultDrive.groupId, respDefaultDrive.driveId, respDefaultDrive.systemFolderId)
    })

    let defaultDrive = new DriveNode({
        ...respDefaultDrive,
        name: respDefaultDrive.driveName,
        children: [homeFolderNode, downloadFolderNode, trashFolderNode, systemFolderNode]
    })
