...
class DockerItemDetailsView implements VirtualDOM {
    ...
    constructor(params: {
        ...
        this.children = [this.instancesListView(), this.buttonNewInstance()]
    }
...
export class DockerItemView implements VirtualDOM {
    public readonly class =
        'd-flex flex-column align-items-center m-2 border rounded p-2 fv-bg-background'
    public readonly children: VirtualDOM[]
    public readonly executable: Executable
    ...
    headerView() {
        return {
            ...
            children: [
                child$(this.executable.appMetadata$, (d) => d.icon),
                {
                    tag: 'span',
                    class: 'mx-2',
                    innerText: attr$(
...
export class AppsDockerView implements VirtualDOM {
    public readonly class =
        'apps-docker-view border rounded d-flex flex-wrap p-1 fv-bg-primary'
    public readonly style = {
        maxWidth: '100%',
    }
                    ...
                    (executable: Executable) =>
                        new DockerItemView({
                            ...
                            instances: runningApps.filter(
                                (app) =>
                                    app.cdnPackage == executable.cdnPackage,
                            ),
                            expanded$,
                            state: this.state,
                        }),
